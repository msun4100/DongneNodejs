...
var getChatRoomMsgByTimeV2 = function (chat_room_id, joined_at, start, display) {
	//overriding getChatRoom function. added 'joined_at' parameter
	return new Promise(function (resolve, reject) {
		var input = [chat_room_id, joined_at, display, (start * display)];
		pool.getConnection(function (err, conn) {
			if (err) { reject(err); }
			var sql = 'SELECT cr.chat_room_id, cr.name, cr.created_at as chat_room_created_at, u.name as username, c.* FROM chat_rooms cr LEFT JOIN messages c ON c.chat_room_id = cr.chat_room_id LEFT JOIN users u ON u.user_id = c.user_id WHERE (cr.chat_room_id = ? AND c.created_at >= ? ) ORDER BY c.created_at DESC LIMIT ? OFFSET ?';
			conn.query(sql, input, function (err, joinedItem) {
				...
				getChatRoomMsgTotal(chat_room_id, function(err, total){
					if(err){
						conn.release();
						reject(err);
					} else {
...
function getChatRoomMsgTotal(chat_room_id, cb) {
	pool.getConnection(function (err, conn) {
		if (err) { cb(err); }
		var sql = 'SELECT COUNT(message_id) as total FROM messages WHERE chat_room_id = ?';
		conn.query(sql, chat_room_id, function (err, results) {
			if (err) {
				conn.release();
				return cb(err, null);
			} 
			conn.release();
			cb(null, results[0].total);
		});
	});
}

// var getChatRoomMsgTotal = function (chat_room_id) {
// 	return new Promise(function (resolve, reject) {
// 		pool.getConnection(function (err, conn) {
// 			if (err) { reject(err); }
// 			var sql = 'SELECT COUNT(message_id) as total FROM messages WHERE chat_room_id = ?';
// 			conn.query(sql, chat_room_id, function (err, result) {
// 				if (err) {
// 					conn.release();
// 					reject(err);
// 				}
...
module.exports = function () {
	...
	return {
		...
		getChatRoomMsgByTimeV2: getChatRoomMsgByTimeV2,
